...
- **Budget bands are stored in dollars, always.** `BUDGETS` is canonical and is what Airtable receives; `BUDGET_LABELS` only changes what the visitor reads. Philippine visitors see peso bands. Location comes from Vercel's `x-vercel-ip-country` header, which `proxy.ts` copies into a `vq_country` cookie; `ContactForm` reads that cookie through `useSyncExternalStore` so the server snapshot stays USD and hydration matches. The relay through a cookie is deliberate — reading the header in a Server Component would make every route carrying `CTABand` render per-request. Browser timezone is the fallback for local dev and non-Vercel hosts — a weak one, since many Philippine machines report `Asia/Singapore`, which is why the visitor can override it with the `USD / PHP` toggle (stored in `vq_currency`, which outranks every detected signal). **That toggle belongs at the top of the form, not beside the Budget label** — in the label row it made the Budget cell taller than Project and Timeline and pushed its select out of line. Never post a peso string — the `Budget` column would mix two currencies and stop being sortable. Peso labels are **derived** from `USD_PHP_RATE`, not typed out: change that one constant and every band follows. It is hand-maintained by design — a live FX call would give a select label a network dependency and a failure mode.
