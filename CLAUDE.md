...
- **Budget bands are stored in dollars, always.** `BUDGETS` is canonical and is what Airtable receives; `BUDGET_LABELS` only changes what the visitor reads. Philippine visitors see peso bands. Currency is resolved in this order, in `ContactForm`: the visitor's own choice (`vq_currency` cookie) → the edge country (`vq_country`, written by `proxy.ts` from Vercel's `x-vercel-ip-country`) → `Asia/Manila` timezone → a Philippine browser locale (`*-PH`, `fil`, `tl`) → USD. Read through `useSyncExternalStore` so the server snapshot stays USD and hydration matches; relayed through a cookie rather than `headers()` because reading a header in a Server Component would make every route carrying `CTABand` render per-request. Never post a peso string — the `Budget` column would mix two currencies and stop being sortable. Peso labels are **derived** from `USD_PHP_RATE`, not typed out: change that one constant and every band follows. It is hand-maintained by design — a live FX call would give a select label a network dependency and a failure mode.
- **The `USD / PHP` toggle belongs at the top of the form, not beside the Budget label.** In that label row it made the Budget cell taller than Project and Timeline and pushed its select out of line with them. **Do not widen the timezone check to the UTC+8 offset** either — many Philippine machines report `Asia/Singapore`, which is why locale is a second signal and the toggle is the real answer; that offset bucket also holds Singapore, Malaysia, Hong Kong, Taiwan, and Perth. The toggle changes labels only; the posted value is always the dollar band.
